...
set_time_limit(0);
// 设置参数
$enter_url = "http://www.net767.com/book/kantucaopan/";
$category = 1;
$localhost = "localhost";
$usn = "root";
$psd = "root";
...
$str2 = mb_convert_encoding("下一页", "GB2312", "UTF-8");
$preg = '/<H1>'.$str.'<\/H1>(.*?)'.$str2.'<\/a>\s*<\/div>/is';
...
for ($i=0; $i < count($link[1]); $i++) { 
	...
	curl_close($ch2[$i]);
	// 匹配内容页第一页正文 
	$preg3 = '/<td(\s*|\s*vAlign=top\s*)class=neirong(.*?)>(.*?)(<div\s*id=\'pagebar\'>|<td\s*width="3\%">\s*<\/td>
)/is';
	preg_match_all($preg3, $cont[$i], $page[$i]);
	$content[$i] = $page[$i][3][0]; //内容页正文（第一页）
	// 匹配内容页分页链接区域
	$preg4 = '/<a\s*href=\'(.*?)\'>(\d+)<\/a>/';
	...
	for ($j=2; $j < ($page_count[$i] + 1); $j++) {
		...
		$paging_content[$i][$j] = $paging_text[$i][$j][3][0]; //内容页正文（分页）
	...
	echo '<h1 style="color:red;">'.$title[$i].'</h1><br>'.$content[$i].'<br>';
	// 连接数据库
